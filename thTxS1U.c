...
void* thTxS1U(void *ptr){
    msgFifo_t *sp_msgCliOut =  &g_msgCliOut[1];    //cli comands
    msgFifo_t *sp_msgCliIn  =  &g_msgCliIn[1];    //cli responses
    msgFifo_t *sp_msgStats  =  &g_msgStats[0];    //stats out
    ...
    while (1) {
        if (state && ((burst) || (burst < 0))){
            ...
            //send some packets
            while(enqueueFifo(&g_msgNet[0], packet));
            portStats.txPkts++;
