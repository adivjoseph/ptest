...
void* thRxSgi(void *ptr){
    msgFifo_t *sp_msgCliOut =  &g_msgCliOut[4];    //cli comands
    msgFifo_t *sp_msgCliIn  =  &g_msgCliIn[4];    //cli responses
    msgFifo_t *sp_msgStats  =  &g_msgStats[3];    //stats out
    ...
    while (1) {
        if((rtc = dequeueFifo(&g_msgNet[0], packet)) != 0) {
            portStats.rxPkts++;
